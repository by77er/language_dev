...
impl<T> List<T> {
    ...
    /// Returns a shared reference to the element at the front of the list "
    pub fn peek(&self) -> Option<&T> {
        self.head.as_ref().map(|node| {
            &node.elem
        })
    }
    /// Returns an exclusive (mutable) reference to the element at the front of the list "
...
impl<T> List<T> {
    /// Consumes this list and returns an iterator
...
impl<T> List<T> {
    /// Returns an iterator of shared (immutable) references to the list's elements
